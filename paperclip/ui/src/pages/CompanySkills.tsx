...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null) {
  ...
  switch (sourceBadge) {
    ...
    case "paperclip":
      return { icon: Paperclip, label: sourceLabel ?? "Finance Anything", managedLabel: "Finance Anything managed" };
    default:
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: "Skill created",
        body: `${skill.name} is now editable in the Finance Anything workspace.`,
      });
    },
    onError: (error) => {
      pushToast({
