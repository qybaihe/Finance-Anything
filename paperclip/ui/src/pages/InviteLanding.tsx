...
function mapInviteAuthFeedback(
  ...
  if (code === "INVALID_EMAIL_OR_PASSWORD") {
    return {
      tone: "error",
      message: t(
        "That email and password did not match an existing Finance Anything account. Check both fields, or create an account first if you are new here.",
      ),
    };
  }
...
export function InviteLandingPage() {
  ...
  const companyDisplayName = companyName || t("this Finance Anything company");
          ...
          <section className={`${panelClassName} space-y-6`}>
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                <p className="text-xs uppercase tracking-[0.24em] text-zinc-500">
                  {t("You've been invited to join Finance Anything")}
                </p>
                <h1 className="mt-2 text-2xl font-semibold">
                  {invite.inviteType === "bootstrap_ceo"
                    ? t("Set up Finance Anything")
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-300">
                  {showsAgentForm
                    ...
                    : requiresHumanAccount
                      ? t("Create your Finance Anything account first. If you already have one, switch to sign in and continue the invite with the same email.")
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="border border-zinc-800 p-3">
                ...
                <div className="mt-1 text-sm text-zinc-100">{invitedByUserName ?? t("Finance Anything board")}</div>
              </div>
              <div className="border border-zinc-800 p-3">
            ...
            {showsAgentForm ? (
                  ...
                  >
                    {joinAdapterOptions.map((type) => (
                      <option key={type} value={type} disabled={!ENABLED_INVITE_ADAPTERS.has(type)}>
                        ...
                        {!ENABLED_INVITE_ADAPTERS.has(type) ? ` ${t("(Coming soon)")}` : ""}
                      </option>
                    ))}
                  </select>
                </label>
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {authMode === "sign_up"
                      ? t("Start with a Finance Anything account. After that, you'll come right back here to accept the invite for {{company}}.", {
                        company: companyDisplayName,
                      })
                      : t("Use the Finance Anything account that already matches this invite. If you do not have one yet, switch back to create account.")}
                  </p>
                </div>
                ...
                <p className="text-xs leading-5 text-zinc-500">
                  {authMode === "sign_up"
                    ? t("Already signed up before? Use the existing-account option instead so the invite lands on the right Finance Anything user.")
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {shouldAutoAcceptHumanInvite
                      ...
                      : t("This will {{action}}.", {
                        action:
                          invite.inviteType === "bootstrap_ceo"
                            ? t("finish setting up Finance Anything")
