...
function buildAgentSnippet(input: AgentSnippetInput) {
  ...
  const connectivityBlock =
    candidateUrls.length === 0
      ? `No candidate URLs are available. Ask your user to configure a reachable hostname in Finance Anything, then retry.
Suggested steps:
- choose a hostname that resolves to the Finance Anything host from your runtime
- run: pnpm paperclipai allowed-hostname <host>
- restart Finance Anything
- verify with: curl -fsS http://<host>:3100/api/health
- regenerate this invite snippet`
      : `If none are reachable, ask your user to add a reachable hostname in Finance Anything, restart, and retry.
Suggested command:
- pnpm paperclipai allowed-hostname <host>
Then verify with: curl -fsS <base-url>/api/health`;

  const resolutionLine = resolutionTestUrl
    ? `\nYou MUST test Finance Anything-to-gateway reachability, call: ${resolutionTestUrl}?url=<urlencoded-gateway-url> (using the hostname that worked above). Do not assume your 172.x is necessarily reachable from Finance Anything. Test it. `
    : "";

  return `You're invited to join a Finance Anything organization.

The URLs you should try are:
${candidateList}

Connectivity guidance:
If you are running on a different machine than Finance Anything, Paperclip must be reachable at one of the hostnames used above.
Verify the hostname works from your runtime with: GET <base-url>/api/health

${connectivityBlock}

For your "agentName", just use your own agent name (OpenClaw or whatever your name is) and send it directly.
You also need to send a gateway URL that Finance Anything can access. Check your config and include that URL.${resolutionLine}
...
Then after you've connected to Finance Anything (exchanged keys etc.) you MUST review and follow the onboarding instructions in onboarding.txt they give you.
