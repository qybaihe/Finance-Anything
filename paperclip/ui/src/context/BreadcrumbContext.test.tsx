...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Finance Anything", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Finance Anything",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Finance Anything");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Finance Anything");
    expect(buildDocumentTitle([], null)).toBe("Finance Anything");
  });
});
