...
describe("MarkdownEditor", () => {
  ...
  it("keeps mention queries active across spaces", () => {
    expect(findMentionMatch("Ping @Finance Anything App", "Ping @Finance Anything App".length)).toEqual({
      trigger: "mention",
      marker: "@",
      query: "Finance Anything App",
      atPos: 5,
      endPos: "Ping @Finance Anything App".length,
    });
  });
  ...
  async function openMentionMenuFor(
    ...
    const option = Array.from(document.body.querySelectorAll('button[type="button"]'))
      .find((node) => node.textContent?.includes("Finance Anything App")) as HTMLButtonElement | undefined;
  ...
  it("accepts mention selection from a touch tap", async () => {
    ...
    expect(handleChange).toHaveBeenCalledWith(
      `[@Finance Anything App](${buildProjectMentionHref("project-123", "#336699")}) `,
    );
