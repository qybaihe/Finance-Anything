...
describe("IssueChatThread system notice routing", () => {
  it("renders authorType=system comments as a SystemNotice rather than a user bubble", () => {
    const comment: IssueChatComment = {
      ...
      authorAgentId: null,
      authorUserId: null,
      body: "Finance Anything needs a disposition before this issue can continue.",
      presentation: {
        kind: "system_notice",
        tone: "warning",
    ...
    expect(container.textContent).toContain("Finance Anything needs a disposition");
  ...
  it("labels system notice source as Finance Anything when no run agent can be resolved", () => {
    const comment: IssueChatComment = {
    ...
    const status = container.querySelector('[role="status"]');
    expect(status).not.toBeNull();
    expect(status?.textContent).toContain("Finance Anything");
    expect(status?.textContent).not.toContain("You");
  });

  it("falls back to Finance Anything in the system notice header when run agent is unknown to agentMap", () => {
    const comment: IssueChatComment = {
    ...
    expect(sourceLink?.textContent).toBe("Finance Anything");
  });
  ...
  it("folds stale successful-run disposition warnings into the activity log disclosure style", () => {
    ...
    expect(row?.textContent).not.toContain("Finance Anything needs a disposition before this issue can continue.");
