...
export const SUCCESSFUL_RUN_HANDOFF_REQUIRED_NOTICE_BODY =
  "Finance Anything needs a disposition before this issue can continue.";
export const SUCCESSFUL_RUN_HANDOFF_EXHAUSTED_NOTICE_BODY =
  "Finance Anything could not resolve this issue's missing disposition automatically. The issue is blocked on a recovery owner.";
...
export function isSuccessfulRunHandoffEscalationComment(text: string) {
  ...
  return trimmed === SUCCESSFUL_RUN_HANDOFF_EXHAUSTED_NOTICE_BODY
    || /^Finance Anything exhausted the bounded successful-run handoff correction\b/i.test(trimmed);
}
